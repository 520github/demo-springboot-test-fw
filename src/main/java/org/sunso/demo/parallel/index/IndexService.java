...
public class IndexService {
    public void runIndexParallelTask(IndexTaskParameter indexTaskParameter) {
        List<BaseParallelResponse> responseList = FailRetryParallelTaskBootstrap.create().setFailRetryNum(3)
                .setExecutor(Executors.newFixedThreadPool(3)).setPoolTaskResultTimeout(100)
                .setTimeUnit(TimeUnit.MILLISECONDS).setBizRequest(indexTaskParameter).isSpringTaskBean(true)
                .executeParallelTask(IndexParallelTaskKey.IndexReadMysqlTask, IndexParallelTaskKey.IndexReadRedisTask,
